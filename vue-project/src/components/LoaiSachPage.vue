<template>
    ...
    <!-- Bảng dữ liệu -->
    <v-data-table :headers="dynamicHeaders" :items="filteredItems" density="compact" :item-key="the_loai_id" height="400px"
        fixed-header :header-props="({ style: 'background-color: #4169E1; color: #ffffff;' })" @click:row="onRowClick"
        hover>
        <template v-slot:item.action="{ item }" v-if="isType1">
            <v-btn icon="mdi-plus-circle" @click="onAddCart(item)"></v-btn>
            <v-btn icon="mdi-pencil" @click="onUpdate(item)"></v-btn>
            <v-btn icon="mdi-delete" @click="onDelete(item)"></v-btn>
        </template>
    </v-data-table>
        ...
        <v-card>
            ...
            <v-card-text>
                <v-form ref="form">
                    ...
                    <v-text-field v-model="currentItem.so_luong_sach" label="Số lượng sách" required></v-text-field>
                    <v-text-field v-model="currentItem.id_ke" label="Kệ" required></v-text-field>
                </v-form>
            </v-card-text>
...
const add = ref(true);
const search = ref('');
const userRole = localStorage.getItem('userRole') || '2'; // '1' cho loại 1, '2' cho loại 2
const isType1 = userRole === '1';

const headers = [
    { title: 'Kệ', align: 'start', key: 'the_loai_id' },
    { title: 'Số lượng loại', align: 'end', key: 'ten_the_loai' },
    { title: 'Tình trạng', align: 'end', key: 'so_luong_sach' },
];

// Tính toán dynamic để lọc dữ liệu dựa trên search
const filteredItems = computed(() => {
    return LoaiSachData.value.filter(item => !item.is_deleted &&
        String(item.the_loai_id).toLowerCase().includes(search.value.toLowerCase())
    );
});

const dynamicHeaders = computed(() => {
    if (isType1) {
        return [...headers, { title: 'Hành động', align: 'end', key: 'action' }];
    }
    return headers;
});
...
function onRowClick(event, item) {
    console.log('Row clicked:', item.internalItem.columns.the_loai_id);
}
</script>
